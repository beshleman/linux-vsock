 ...
 *
 * Each skb is freed.
 * Only to be called when holding a lock that protects the queue.
 *
 * Returns the count of skbs that were reply packets.
 */
int virtio_transport_remove_skbs(void *vsk, struct sk_buff_head *queue)
{
	int cnt;
	struct sk_buff *skb, *tmp;
...
EXPORT_SYMBOL_GPL(virtio_transport_remove_skbs);
