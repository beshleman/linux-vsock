...
static __init int vsock_dev_register(void)
{
	int i;
	int ret;
...
static __exit void vsock_dev_unregister(void)
{
	struct vsock_dev *vdev;
	int i;
...
module_init(vsock_dev_register);
module_exit(vsock_dev_unregister);

MODULE_LICENSE("GPL v2");
MODULE_AUTHOR("Bobby Eshleman <bobby.eshleman@bytedance.com>");
MODULE_DESCRIPTION("Vsock device. Used by vsock sockets.");
MODULE_ALIAS_RTNL_LINK("vsock_dev");
