...
/*
 * Merge a new packet into an old packet.
 *
 * Upon returning, the old packet will have been appended with the contents of
 * the new packet and it will be updated with the latest socket-wide stats from
 * the new packet.
 */
static void
virtio_transport_skb_merge_new_into_old(struct sk_buff *old, struct sk_buff *new)
{
	u32 len;

	len = le32_to_cpu(vsock_hdr(old)->len);
	len += new->len;
	vsock_hdr(old)->len = cpu_to_le32(len);

	/* Provide the most recent buffer stats */
...
static void
virtio_transport_merge_queue(struct sk_buff_head *queue)
{
	/*
	...
	while (skb_queue_len(queue) > 1) {
		struct sk_buff *old, *new;

		new = __skb_dequeue_tail(queue);
		...
		virtio_transport_skb_merge_new_into_old(old, new);
		kfree_skb(new);
	}
...
virtio_transport_send_pkt(struct sk_buff *skb)
{
	...
	virtio_transport_merge_queue(&vsock->send_pkt_queue);
...
virtio_transport_cancel_pkt(struct vsock_sock *vsk)
{
	...
	cnt = virtio_transport_remove_skbs(vsk, &vsock->send_pkt_queue);
...
static void virtio_vsock_rx_fill(struct virtio_vsock *vsock)
{
	...
	do {
		...
		memset(skb->head, 0, sizeof(struct virtio_vsock_metadata) + sizeof(struct virtio_vsock_hdr));
