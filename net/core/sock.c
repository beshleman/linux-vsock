...
sock_devmem_dontneed(struct sock *sk, sockptr_t optval, unsigned int optlen)
{
	...
	for (i = 0; i < num_tokens; i++) {
		...
		last_token = tokens[i].token_start + tokens[i].token_count - 1;
		if (WARN_ON_ONCE(last_token >= sk->sk_rx_binding->dmabuf->size / PAGE_SIZE))
			continue;
		...
		for (j = 0; j < tokens[i].token_count; j++) {
			...
			niov = sk->sk_rx_binding->rx_vec[token];
			netmem = net_iov_to_netmem(niov);

			if (!netmem || WARN_ON_ONCE(!netmem_is_net_iov(netmem)))
				continue;

			if (WARN_ONCE(!sk->sk_rx_binding->rx_vec_uref[token],
				      "user released token too many times"))
				continue;

			sk->sk_rx_binding->rx_vec_uref[token]--;
