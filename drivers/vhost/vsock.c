...
static struct vhost_vsock *vhost_vsock_get(u32 guest_cid, struct net *net)
{
	...
	hash_for_each_possible_rcu(vhost_vsock_hash, vsock, hash, guest_cid) {
		...
		if (other_cid == guest_cid) {
			...
			/* If this is a non-ns vsock, then this is is maybe the
			 * one we want. Only use it if we don't find any with a
			 * matching net. This avoids a second search through
			 * the hashlist.
			 */
			if (!vsock->net)
				fallback = vsock;
